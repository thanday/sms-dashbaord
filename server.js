...
app.get('/dashboard/program/:id', async (req, res) => {
    ...
    res.render('program-detail', { programName: result.rows[0].name, programId: req.params.id });
});
...
app.get('/api/download-inaameh-zip', async (req, res) => {
    ...
    try {
        ...
        const keywordsResult = await pool.query(
            'SELECT id, name FROM keywords WHERE program_id = $1', [programId]
        );
        ...
        for (const kw of keywords) {
            ...
            archive.append(uniqueNumbers.join('\r\n'), { name: `${kw.name}.txt` });
        }
...
app.get('/api/keywords/:programId', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM keywords WHERE program_id = $1 ORDER BY active_time ASC, name ASC', [req.params.programId]);
        res.json(result.rows);
...
app.post('/api/keywords', async (req, res) => {
    try {
        ...
        const timeValue = active_time && active_time.trim() !== "" ? active_time : null;
...
app.get('/api/global-daily-stats', async (req, res) => {
    const result = await pool.query("SELECT TO_CHAR(received_at, 'YYYY-MM-DD') as day, COUNT(id) as count FROM sms_logs GROUP BY day ORDER BY day ASC");
    res.json(result.rows);
});
...
app.post('/upload', upload.single('file'), (req, res) => {
    ...
    fs.createReadStream(req.file.path)
        ...
        .on('end', async () => {
            let dIdx = -1, mIdx = -1, msgIdx = -1;
            for (let i = 0; i < rows.length; i++) {
                const line = rows[i].map(c => String(c).toLowerCase().trim());
                dIdx = line.findIndex(c => c.includes('date') || c.includes('time'));
                mIdx = line.findIndex(c => c.includes('mobile') || c.includes('msisdn') || c.includes('number'));
                msgIdx = line.findIndex(c => c.includes('message') || c.includes('response'));

                if (dIdx !== -1 && mIdx !== -1) {
                    for (let j = i + 1; j < rows.length; j++) {
                        const dataRow = rows[j];
                        let rDate = dataRow[dIdx], rMsisdn = dataRow[mIdx];
                        let rMsg = msgIdx !== -1 ? dataRow[msgIdx] : "";
                        
                        if (rDate && rMsisdn) {
                            let cDate = rDate.trim();
                            if (cDate.includes('/') || (operator === 'Dhiraagu' && cDate.includes('-'))) {
                                const sep = cDate.includes('/') ? '/' : '-';
                                const pts = cDate.split(' '), dPts = pts[0].split(sep);
                                if (dPts[0].length <= 2) cDate = `${dPts[2]}-${dPts[1]}-${dPts[0]} ${pts[1] || '00:00:00'}`;
                            }
                            try {
                                await pool.query(
